...
KG_dotplot = function(seurat_object, genes, cols = c("blue4", "red2"),
  ...
  df = DotPlot(object = seurat_object, features = genes)$data

  # Construct the plot
  plot = ggplot(df, aes(x = reorder(features.plot, desc(features.plot)), y = reorder(id, desc(id)))) +
    ...
    theme_bw() +
    ggtitle(title) +
    theme(plot.title = element_text(hjust = 0.5, size = 18, family = font),
