...
KG_matchGEXtoVDJ = function(gex_data, 
  ...
  if(multiple_samples){
    ...
    for(i in 1:length(vdj_data)){
      ...
      matching_seqs = intersect(Seurat::Cells(gex_data[[i]]))
      if(print_numbers){
        cat(paste0("Total number of V(D)J sequences in ", names(vdj_data)[i], ": ", length(nseq_vdj), "\n"))
        cat(paste0("Number of matching sequences in ", names(vdj_data)[i], ": ", length(matchings_seqs), "(", length(matching_seqs) / nseq_vdj, "% of V(D)J sequences)\n"))
      }
      if(save_filtered){
    ...
    if(print_numbers){
      ...
      cat(paste0("Number of matching sequences: ", length(matchings_seqs), "(", length(matching_seqs) / nseq_vdj, "% of V(D)J sequences)\n"))
    }
    if(save_filtered){
      openxlsx::write.xlsx(dplyr::filter(vdj_data, !cell_id %in% matching_seqs), 
                           paste0(deparse(vdj_data), "_sequences_not_matching_GEX.xlsx"))
    }
  }
  gc()
  return(vdj_data)
